...
class TagScriptInterpreter:
    ...
    def load_stdlib_net(self, name_or_url):
        ...
        if s.lower().startswith('http://') or s.lower().startswith('https://'):
            ...
            # treat as module name in default repo
            name = s
        ...
        try:
            ...
            raise RuntimeError(f"Downloaded module parse failed: {e}")
    ...
    def _exec_node(self, node: TSNode):
        ...
        if tag in ('aslong', 'while'):
            ...
            while True:
                ...
                if not ok:
                    break
                # yield a bit to allow GUI to process the queue and avoid tight spin
                time.sleep(0.001)
        ...
        if tag == 'useit':
            ...
            if callname and '.' in callname:
                ...
                fn = mod.get(fname) if isinstance(mod, dict) else getattr(mod, fname, None)
                if fn is None:
...
def main():
    ...
    <start>
        ...
        <!-- example bring_net (remote .tag module) -->
