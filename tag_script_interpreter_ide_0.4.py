...
File: tag_script_interpreter_ide_0.3.py

Run: python tag_script_interpreter_ide_0.3.py
...
- Safe expression evaluator (AST-based) — supports a small set of safe builtin calls
- Integrated console input area (interpreter blocks until user responds). Works for worker-thread runs;
  if the interpreter is run on the GUI thread a modal dialog is used to avoid deadlock.
- Trims input and properly detects <take/> children even when whitespace/text nodes surround them.
...
class SafeEvaluator:
    """
    Minimal safe evaluator using AST. Supports expressions and a
    small, explicitly allowed set of builtin calls (no attribute
    access or arbitrary calls).
    """
    ...
    # SAFE_BUILTINS maps allowed function names to the actual callable.
    SAFE_BUILTINS = {
    ...
    def _eval_node(self, node):
        ...
        if isinstance(node, ast.Call):
            # Only allow calling a restricted set of builtin names or extras (no attribute calls)
            func_node = node.func
            if isinstance(func_node, ast.Name):
                fname = func_node.id
                if fname in self.SAFE_BUILTINS:
                    fn = self.SAFE_BUILTINS[fname]
                elif fname in self.extras:
                    fn = self.extras[fname]
                else:
                    raise RuntimeError(f"Call to unknown or disallowed function '{fname}'")
            else:
                # disallow attribute calls, lambda calls, etc.
                raise RuntimeError("Call expressions must be simple builtin/extras names")

            # evaluate args and keywords
            args = [self._eval_node(a) for a in node.args]
            kwargs = {}
            for kw in node.keywords:
                if kw.arg is None:
                    # no **kwargs allowed
                    raise RuntimeError("Keyword unpacking is not allowed")
                kwargs[kw.arg] = self._eval_node(kw.value)

            try:
                return fn(*args, **kwargs)
            except Exception as e:
                raise RuntimeError(f"Error calling function {fname}: {e}")
...
class TagScriptInterpreter:
    ...
    def _first_nontext_child(self, node: TSNode):
        """Return first child whose tag is not '#text', or None."""
        if not node.children:
            return None
        for ch in node.children:
            if ch.tag != '#text':
    ...
    def _exec_node(self, node: TSNode):
        ...
        if tag == 'letssay':
            ...
            if 'value' in node.attrib:
                ...
                # check for a non-text child (e.g. a <take/> or other tag)
                nontext = self._first_nontext_child(node)
                if nontext and nontext.tag.lower() == 'take':
                    val = self._exec_node(nontext)
                else:
                    # fallback to first text child if present
                    if node.children and node.children[0].tag == '#text':
                        val = self._interp_text(node.children[0].text)
                    else:
                        val = None
            # normalize string results to remove accidental indentation/newlines
            if isinstance(val, str):
                val = val.strip()
            self.env[name] = val
            return None
        ...
        if tag == 'isnow':
            name = node.attrib.get('name')
            if not name:
                # similar robust handling: look for first non-text child or text content
                nontext = self._first_nontext_child(node)
                content = ''
                if nontext and nontext.tag == '#text':
                    content = nontext.text.strip()
...
class TagScriptIDE(tk.Tk):
    def __init__(self):
        ...
        # pending input state (for new in-console input)
        self.pending_input_resp_q = None
    ...
    def _make_widgets(self):
        ...
        btn_clear = tk.Button(toolbar, text='Clear Console', command=self._clear_console)
        ...
        # Input area integrated into right_frame (so interpreter take() calls can be answered here)
        input_frame = tk.Frame(right_frame)
        input_frame.pack(fill='x', pady=(4, 4))
        self._input_prompt_label = tk.Label(input_frame, text='', anchor='w', width=40)
        self._input_prompt_label.pack(side='left', padx=(0, 4), expand=True, fill='x')
        self.input_entry = tk.Entry(input_frame, state='disabled')
        self.input_entry.pack(side='left', fill='x', expand=True)
        self.input_entry.bind('<Return>', self._submit_input)
        self.input_button = tk.Button(input_frame, text='Send', state='disabled', command=self._submit_input)
        self.input_button.pack(side='left', padx=(4, 0))
    ...
    def _ide_output(self, text):
        # Put as 'out' so it's processed in GUI thread and displayed in console
        self.run_queue.put(('out', str(text)))

    # Request input from user: supports both main-thread and worker-thread callers
    def request_input(self, prompt=None):
        """
        Called by the interpreter when it needs input.

        - If called from the GUI (main) thread we show a simpledialog.askstring
          (modal) and return the user's value. This allows running the interpreter
          on the main thread for quick tests without deadlocking the GUI.
        - If called from a worker thread we put an 'input_request' on the run_queue
          with a resp_q and block on resp_q.get() until the GUI puts the answer.
          The worker waits in a loop with a short timeout so it can respond to
          stop_requested.
        """
        # If called from the GUI (main) thread, use a modal dialog so the UI can
        # handle the prompt/entry immediately.
        if threading.current_thread() == threading.main_thread():
            try:
                resp = simpledialog.askstring('Input requested', prompt or 'Input:')
        ...
        # Debug message so user can see the input request
        try:
            self.run_queue.put(('out', f'[debug] input requested: {prompt or ""}'))
        except Exception:
            pass
        # Ask GUI to enable integrated input area
        self.run_queue.put(('input_request', (prompt or '', resp_q)))

        # Wait for response, but loop so we can honor stop_requested.
        while True:
            if self.interpreter.stop_requested:
                # If stopped, return None to caller
                return None
            try:
                # Wait with short timeout to allow checking stop_requested regularly
                result = resp_q.get(timeout=0.1)
    ...
    def stop_script(self):
        if self.run_thread and self.run_thread.is_alive():
            ...
            # if input was pending, release the GUI area
            if self.pending_input_resp_q:
                try:
                    # put a sentinel so worker won't hang waiting (worker checks stop_requested too)
                    self.pending_input_resp_q.put(None, timeout=0.1)
                except Exception:
                    pass
    ...
    def _enable_input_area(self, prompt, resp_q):
        try:
            self.pending_input_resp_q = resp_q
            self.pending_input_prompt = prompt
            self._input_prompt_label.config(text=prompt or '')
    ...
    def _disable_input_area(self):
        try:
            self.pending_input_resp_q = None
            self.pending_input_prompt = None
            self._input_prompt_label.config(text='')
            self.input_entry.delete(0, 'end')
    ...
    def _submit_input(self, event=None):
        """Called when the user presses Enter or clicks Send in the integrated input area."""
        if not self.pending_input_resp_q:
            return
        val = self.input_entry.get()
        # Echo input to console
        try:
            self.console.insert('end', '> ' + str(val) + '\n')
            self.console.see('end')
        except Exception:
            pass
        try:
            # strip whitespace/newlines
            to_put = val.strip() if isinstance(val, str) else val
            self.pending_input_resp_q.put(to_put, timeout=1)
        except Exception:
            try:
                self.pending_input_resp_q.put(None, timeout=0.1)
            except Exception:
            ...
            self._disable_input_area()
            # update variables view in case input modified env
            self.run_queue.put(('vars', None))
            # debug echo
            try:
                self.run_queue.put(('out', f'[debug] input submitted: {repr(to_put)}'))
            except Exception:
                pass
    ...
    def _pump_queue(self):
        try:
            while True:
                ...
                if typ == 'out':
                    # normal output from interpreter
                    ...
                    # Instead of a modal dialog, present the prompt in the input area
                    # and enable the entry so user can type the response and press Enter
                    self._enable_input_area(prompt or 'Input:', resp_q)
        except queue.Empty:
            pass
        self.after(50, self._pump_queue)
    ...
    def _on_close(self):
        if messagebox.askokcancel('Quit', 'Exit TagScript IDE?'):
            # If a script is running, request stop and also release any pending input
            if self.run_thread and self.run_thread.is_alive():
                self.interpreter.stop_requested = True
                if self.pending_input_resp_q:
                    try:
...
def main():
    ...
    sample = '''<page>
    <start>
        <write>Welcome to TagScript I/O demo!</write>

        <write>What's your name?</write>
        <letssay name="name">
            <take prompt="Name:"/>
        </letssay>
        <write>Hello {{ name }}</write>

        <write>Enter a number to count to:</write>
        <letssay name="n">
            <take prompt="Number:"/>
        </letssay>

        <!-- convert input string to int and loop -->
        <letssay name="n" value="int(n)"/>
        <letssay name="i" value="1"/>
        <aslong value="i <= n">
            <write>Count: {{ i }}</write>
            <isnow name="i" value="i + 1"/>
        </aslong>

        <write>All done — thanks {{ name }}!</write>
    </start>
</page>
'''
